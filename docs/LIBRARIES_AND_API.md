...
로그인은 아이디 기반이며, 이메일은 회원가입 시에만 수집해 비밀번호 재설정 용도로 사용한다(기획서 기능명세서 #2, #3 기준). 화면설계서 로그인/회원가입 목업(Slide 27, 28) 기준으로 아이디는 영문+숫자 8-12자, 비밀번호는 영문+숫자+특수기호 8자 이상 형식을 클라이언트에서 검증하고(`src/lib/authValidation.ts`), 회원가입 폼에 비밀번호 확인 필드를 둔다(서버에는 전송하지 않음). 로그인 폼의 "자동 로그인" 체크박스는 서버 요청과 무관하게 프론트에서 토큰 저장 위치만 바꾼다 — 체크 시 `localStorage`(브라우저 재시작 후에도 유지), 미체크 시 `sessionStorage`(탭 종료 시 소멸).
